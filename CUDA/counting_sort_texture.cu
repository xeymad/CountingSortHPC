...
__global__ void kernelMaxMin(int array_size,int *maxmin_out){
  // BANK CONFLICTS
  ...
  sdata[tid] = tex1Dfetch(array_texture,i);
  __syncthreads();
...
__global__ void kernelCountingArraySum(int *counting_array,int array_size, int min) {
  ...
  if(i<array_size){
    atomicAdd(&counting_array[tex1Dfetch(array_texture,i) - min],1);
  }
...
void deviceCountingSort(int *array, int array_size) {
  ...
  int size = sizeof(int);
  size_t sizet = allocation*size;
  int *dev_maxmin_out;
  int *maxmin_out = (int*)malloc(gridsize*2*size);
  assert(maxmin_out!=NULL);
  printf("The allocation is %d\n",allocation);
  cudaMalloc((void**)&deviceArray, allocation*size);
  HandleError(cudaMemcpy(deviceArray,array,sizet,cudaMemcpyHostToDevice));
  if(allocation>array_size){
    //Copy array[0] to the last element
    HandleError(cudaMemcpy(deviceArray+array_size,&array[0],size,cudaMemcpyHostToDevice));
  }
  cudaChannelFormatDesc vChannel = cudaCreateChannelDesc<int>();
  HandleError(cudaBindTexture(0, array_texture, deviceArray, vChannel));
  ...
  kernelMaxMin<<<dimGrid,dimBlock,THREADxBLOCK*size>>>(allocation,dev_maxmin_out);
  ...
  cudaEventRecord(start, 0);
  // kernel launch
  kernelCountingArraySum<<<dimGrid, dimBlock>>>(dev_counting_array,allocation,min);
  ...
  HandleError(cudaMemcpy(counting_array, dev_counting_array, c_len*sizeof(int), cudaMemcpyDeviceToHost));
  // free memory on device and texture
  HandleError(cudaUnbindTexture(array_texture));
